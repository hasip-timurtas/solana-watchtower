...
pub use watchtower_subscriber::{ProgramEvent, EventData, EventType};
...
impl Rule for LiquidityDropRule {
    ...
    async fn evaluate(&self, event: &ProgramEvent, context: &RuleContext) -> RuleResult {
        ...
        if let EventData::TokenTransfer { amount, .. } = &event.data {
            ...
            let window_start = event.timestamp - chrono::Duration::seconds(self.window_seconds as i64);
            let recent_transfers: Vec<_> = context.recent_events
                .iter()
            ...
            if !recent_transfers.is_empty() {
                ...
                if total_amount_out >= self.min_liquidity {
                    ...
                    if drop_pct >= self.threshold_pct {
                        ...
                        result.metadata.insert("drop_percentage".to_string(), drop_pct.into());
                        result.metadata.insert("amount".to_string(), (*amount).into());
                        result.suggested_actions.push("Monitor for market manipulation".to_string());
                        result.suggested_actions.push("Check for flash loan attacks".to_string());
                    }
                }
            }
        }
    ...
    async fn evaluate(&self, event: &ProgramEvent, context: &RuleContext) -> RuleResult {
        ...
        if let EventData::TokenTransfer { amount, .. } = &event.data {
            ...
            if *amount >= self.amount_threshold {
                ...
                result.metadata.insert("amount".to_string(), (*amount).into());
                result.suggested_actions.push("Verify transaction legitimacy".to_string());
            }
            ...
            if let Some(tvl) = context.metrics.get("total_value_locked") {
                ...
                if pct_of_tvl >= self.tvl_threshold_pct {
                    ...
                    result.metadata.insert("tvl_percentage".to_string(), pct_of_tvl.into());
                    result.suggested_actions.push("Monitor for large player activity".to_string());
                }
            }
        }
            ...
            if let serde_json::Value::Number(price) = oracle_price {
                if let Some(reference_price) = context.metrics.get(&self.reference_oracle) {
                    ...
                    if deviation_pct >= self.max_deviation_pct {
                        ...
                        result.metadata.insert("deviation_percentage".to_string(), deviation_pct.into());
                        result.metadata.insert("current_price".to_string(), price_val.into());
                        result.metadata.insert("reference_price".to_string(), (*reference_price).into());
                        result.suggested_actions.push("Halt trading immediately".to_string());
                        result.suggested_actions.push("Investigate oracle data source".to_string());
                        result.suggested_actions.push("Contact oracle provider".to_string());
                    }
                }
            }
        }
...
impl FailureRateRule {
    pub fn new(max_failure_rate_pct: f64, min_transaction_count: usize, window_seconds: u64) -> Self {
        Self {
...
impl Rule for FailureRateRule {
    ...
    async fn evaluate(&self, event: &ProgramEvent, context: &RuleContext) -> RuleResult {
        ...
        if event.is_transaction() {
            let window_start = event.timestamp - chrono::Duration::seconds(self.window_seconds as i64);
            let recent_transactions: Vec<_> = context.recent_events
                .iter()
            ...
            if recent_transactions.len() >= self.min_transaction_count {
                ...
                if failure_rate >= self.max_failure_rate_pct {
                    ...
                    result.message = Some(format!(
                        ...
                        failure_rate, failed_count, recent_transactions.len()
                    ));
                    result.confidence = (failure_rate / 100.0).min(1.0);
                    result.metadata.insert("failure_rate".to_string(), failure_rate.into());
                    result.metadata.insert("failed_count".to_string(), failed_count.into());
                    result.metadata.insert("total_count".to_string(), recent_transactions.len().into());
                    result.suggested_actions.push("Check program logic for errors".to_string());
                    result.suggested_actions.push("Monitor network congestion".to_string());
                }
            }
        }
