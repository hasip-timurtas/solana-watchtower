...
impl AlertManager {
    ...
    pub async fn send_alert(&self, mut alert: Alert) -> AlertResult<()> {
        ...
        info!("Alert generated: {} - {} ({})", alert.id, alert.message, alert.severity.as_str());
    ...
    pub async fn acknowledge_alert(&self, alert_id: &str) -> AlertResult<()> {
        if let Some(mut alert_entry) = self.alerts.get_mut(alert_id) {
            ...
            Err(AlertError::NotFound { id: alert_id.to_string() })
        }
    }
    ...
    async fn find_duplicate(&self, alert: &Alert) -> Option<Alert> {
        let window_start = Utc::now() - chrono::Duration::seconds(self.config.deduplication_window_seconds as i64);
    ...
    async fn update_statistics(&self, alert: &Alert) {
        ...
        *stats.by_rule.entry(alert.rule_name.clone()).or_insert(0) += 1;
        ...
        // Update by program
        *stats.by_program.entry(alert.program_name.clone()).or_insert(0) += 1;
    }
    ...
    async fn cleanup_alerts(&self) {
        ...
        let cutoff_time = Utc::now() - chrono::Duration::seconds(self.config.auto_resolve_age_seconds as i64);
        ...
        if self.alerts.len() > self.config.max_active_alerts {
            ...
            let mut oldest_alerts: Vec<_> = self.alerts
                .iter()
